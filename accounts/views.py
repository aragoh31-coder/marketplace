    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            AuditLog.objects.create(
                ...
                details={"registration_method": "standard", "username": username, "email_provided": bool(user.email)},
                risk_score=0,
            )
        ...
        if form.is_valid():
            ...
            if user:
                ...
                if user.pgp_login_enabled and user.pgp_public_key:
                    ...
                    encrypt_result = pgp_service.encrypt_message(challenge_message, user.pgp_fingerprint)
                    ...
                    LoginHistory.objects.create(
                        user=user,
                        ip_hash=hashlib.sha256("privacy_protected".encode()).hexdigest(),  # No real IP logging
                        user_agent=request.META.get("HTTP_USER_AGENT", "")[:200],
                        success=True,
                    )
...
def profile_view(request):
    ...
    try:
        ...
        current_orders = Order.objects.filter(buyer=user, status__in=["created", "paid", "shipped"]).count()
    ...
    try:
        ...
        active_disputes = Dispute.objects.filter(order__buyer=user, status="open").count()
    except:
        active_disputes = 0
...
def pgp_settings(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            encrypt_result = pgp_service.encrypt_message(verification_message, fingerprint)
