...
"""
    ...
    def validate_key_format(self, key_data):
        ...
        if not has_begin or not has_end:
            return {"success": False, "error": "Invalid PGP key format. Key must include BEGIN and END markers."}
        ...
        for line in lines:
            ...
            if any(re.search(pattern, line, re.IGNORECASE) for pattern in begin_patterns):
                ...
                if re.match(r"^[A-Za-z0-9+/=]+$", line):
                    ...
                    cleaned_line = "".join(
                        c for c in line if c in "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/="
                    )
        ...
        if len(normalized_lines) < 3:
            return {"success": False, "error": "Key appears to be incomplete or corrupted"}

        return {"success": True, "normalized_key": normalized_key, "original_key": key_data}
        ...
        for subkey in subkeys:
            if isinstance(subkey, dict):
                formatted_subkey = {
                    ...
                    "created": (
                        datetime.fromtimestamp(int(subkey.get("date", 0))).strftime("%Y-%m-%d")
                        if subkey.get("date")
                        else "Unknown"
                    ),
                }
            ...
            if import_result.count > 0:
                ...
                if key_info and key_info.get("success"):
                    ...
                    if caps.get("is_expired"):
                        return {"success": False, "error": "This PGP key has expired and cannot be used"}

                    if caps.get("is_revoked"):
                        return {"success": False, "error": "This PGP key has been revoked and cannot be used"}
                ...
                if "invalid" in error_msg.lower():
                    return {"success": False, "error": "Invalid key format or corrupted key data"}
                elif "duplicate" in error_msg.lower():
                    return {"success": False, "error": "This key is already imported"}
                else:
                    return {"success": False, "error": f"Key import failed: {error_msg}"}
    ...
    def encrypt_message(self, message, recipient_fingerprint):
        ...
        try:
            ...
            if key_info and key_info.get("success"):
                ...
                if caps.get("is_expired"):
                    return {"success": False, "error": "Cannot encrypt: recipient key has expired"}

                if caps.get("is_revoked"):
                    return {"success": False, "error": "Cannot encrypt: recipient key has been revoked"}

                if not caps.get("can_encrypt") and not caps.get("has_encryption_subkey"):
                    return {"success": False, "error": "Cannot encrypt: recipient key does not support encryption"}

            encrypted_data = self.gpg.encrypt(
                message, recipients=[recipient_fingerprint], always_trust=True, armor=True, symmetric=False
            )
            ...
            if encrypted_data.ok:
                ...
                if encrypted_message.startswith("-----BEGIN PGP MESSAGE-----"):
                    ...
                    return {"success": False, "error": "Encrypted data is not in proper PGP format"}
            else:
                error_msg = str(encrypted_data.stderr)
                logger.error(f"Encryption failed: {error_msg}")

                if "no valid recipients" in error_msg.lower():
                    return {"success": False, "error": "No valid recipients found. Please check the key fingerprint."}
                elif "unusable public key" in error_msg.lower():
                    return {"success": False, "error": "The public key is not suitable for encryption."}
                else:
                    return {"success": False, "error": f"Encryption failed: {error_msg}"}
    ...
    def extract_message_from_signature(self, signed_message):
        ...
        try:
            ...
            if not extracted_message:
                return {"success": False, "error": "No message content found in signed message"}
            ...
            return {"success": True, "message": extracted_message, "verification": verification_result}
            ...
            for key in keys:
                if key["fingerprint"] == fingerprint:
                    ...
                    return {
                        ...
                        "created": (
                            datetime.fromtimestamp(int(key.get("date", 0))).strftime("%Y-%m-%d")
                            if key.get("date")
                            else "Unknown"
                        ),
                        "expires": (
                            datetime.fromtimestamp(int(key.get("expires", 0))).strftime("%Y-%m-%d")
                            if key.get("expires")
                            else "Never"
                        ),
                        "capabilities": caps,
    ...
    def decrypt_message(self, encrypted_message, passphrase=None):
        ...
        try:
            ...
            if decrypted_data.ok:
                ...
                return {"success": False, "error": f"Decryption failed: {decrypted_data.stderr}"}
        except Exception as e:
    ...
    def create_signed_message(self, message, keyid=None, passphrase=None):
        ...
        try:
            ...
            if signed_data:
                return {"success": True, "signed_message": str(signed_data), "fingerprint": signed_data.fingerprint}
            else:
