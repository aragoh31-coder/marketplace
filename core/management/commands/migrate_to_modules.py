...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('Starting migration to modular system...')
        )
    ...
    def _get_apps_to_migrate(self, specified_apps):
        ...
        marketplace_apps = [
            'accounts', 'wallets', 'vendors', 'products', 'orders',
            'disputes', 'messaging', 'support', 'adminpanel'
        ]
        
        installed_marketplace_apps = [
            app for app in marketplace_apps
    ...
    def _migrate_apps(self, apps_to_migrate, force=False):
        ...
        for app_name in apps_to_migrate:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✓ {app_name} migrated successfully')
                )
            except Exception as e:
                if force:
                    self.stdout.write(
        ...
        for component in required_components:
            try:
                if component == 'models':
                    ...
                    if not models:
                        self.stdout.write(
                            self.style.WARNING(f'  Warning: {app_name} has no models')
                        )
                ...
                self.stdout.write(
                    self.style.WARNING(f'  Warning: {app_name} missing {component} module')
                )
    ...
    def _validate_system(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'Modular system not available: {e}')
            )
        except Exception as e:
            self.stdout.write(
    ...
    def _get_migration_status(self):
        ...
        try:
            ...
            status = {
                'total_apps': len(marketplace_apps),
                'modularized': [],
                'pending': [],
                'failed': []
            }
    ...
    def _show_migration_status(self):
        ...
        if 'error' in status:
            self.stdout.write(
                self.style.ERROR(f'Cannot determine migration status: {status["error"]}')
            )
            return
