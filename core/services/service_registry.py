...
class ServiceRegistry:
    ...
    def get_available_services(cls) -> Dict[str, BaseService]:
        ...
        return {name: service for name, service in cls._services.items() if service.is_available()}

    @classmethod
    def get_healthy_services(cls) -> Dict[str, BaseService]:
        """Get all healthy services."""
        return {name: service for name, service in cls._services.items() if service.get_health_status()["healthy"]}

    @classmethod
    def get_service_by_type(cls, service_type: Type[BaseService]) -> List[BaseService]:
        """Get all services of a specific type."""
        return [service for service in cls._services.values() if isinstance(service, service_type)]
