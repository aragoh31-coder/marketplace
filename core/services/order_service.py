...
class OrderService(BaseService):
    ...
    def create_order(
        self, user_id: str, vendor_id: str, items: List[Dict], shipping_address: str, **kwargs
    ) -> Tuple[Any, bool, str]:
        """Create a new order."""
        try:
            ...
            with transaction.atomic():
                ...
                for item in items:
                    ...
                    order_items.append(
                        {"product": product, "quantity": quantity, "price": product.price, "total": item_total}
                    )
                ...
                if available_balance < total_amount:
                    return (
                        None,
                        False,
                        f"Insufficient balance. Required: {total_amount}, Available: {available_balance}",
                    )
                ...
                for item_data in order_items:
                    product_service.update_product_stock(str(item_data["product"].id), -item_data["quantity"], "adjust")
    ...
    def update_order_status(
        self, order_id: str, new_status: str, admin_user_id: str = None, notes: str = ""
    ) -> Tuple[bool, str]:
        """Update order status."""
        try:
    ...
    def _release_funds_to_vendor(self, order: Any) -> None:
        ...
        try:
            ...
            success, msg = wallet_service.release_from_escrow(
                str(order.user_id), order.currency.lower(), order.total_amount, str(order.id)
            )

            if success:
                # Add funds to vendor's wallet
                wallet_service.add_funds(
                    str(order.vendor_id), order.currency.lower(), order.total_amount, f"order_{order.id}"
                )
        ...
        try:
            ...
            if success:
                ...
                for item in order.items.all():
                    product_service.update_product_stock(str(item.product_id), item.quantity, "adjust")
    ...
    def _log_order_status_change(
        self, order_id: str, old_status: str, new_status: str, admin_user_id: str = None, notes: str = ""
    ) -> None:
    ...
    def get_order_summary(self, order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "processing_started": (
                    order.processing_started.isoformat()
                    if hasattr(order, "processing_started") and order.processing_started
                    else None
                ),
                "shipped_at": (
                    order.shipped_at.isoformat() if hasattr(order, "shipped_at") and order.shipped_at else None
                ),
                "completed_at": (
                    order.completed_at.isoformat() if hasattr(order, "completed_at") and order.completed_at else None
                ),
                "cancelled_at": (
    ...
    def get_order_statistics(self, user_id: str = None, vendor_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(12):
                ...
                month_orders = queryset.filter(created_at__gte=month_start, created_at__lte=month_end)
