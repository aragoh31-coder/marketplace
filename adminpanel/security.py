...
class AdminSecurityManager:
    ...
    def record_failed_attempt(self, user, attempt_type="login"):
        ...
        if attempts >= self.max_failed_attempts:
            ...
            self._log_security_event(user, "admin_lockout", {"attempt_type": attempt_type, "failed_attempts": attempts})
    ...
    def generate_pgp_challenge(self, user):
        ...
        cache.set(
            cache_key,
            {"challenge": challenge_string, "hash": challenge_hash, "timestamp": challenge_data["timestamp"]},
            600,
        )  # 10 minutes
    ...
    def validate_admin_session(self, request):
        ...
        current_ua_hash = hashlib.sha256(request.META.get("HTTP_USER_AGENT", "").encode()).hexdigest()[:16]
...
class TripleAuthenticator:
    ...
    def start_authentication(self, user, request):
        ...
        auth_state = {"user_id": user.id, "step": 1, "completed_steps": [], "started_at": time.time()}
    ...
    def verify_step(self, auth_token, step, verification_data):
        ...
        if step == 1:  # Primary password
            success, message = self._verify_primary_password(auth_state["user_id"], verification_data.get("password"))
            ...
            success, message = self._verify_pgp_challenge(auth_state["user_id"], verification_data.get("pgp_response"))
