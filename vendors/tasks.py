...
def create_low_stock_notification(self, vendor_id, product_ids):
    ...
    try:
        ...
        if len(product_names) > 3:
            message = (
                f"Products running low on stock: {', '.join(product_names[:3])} and {len(product_names) - 3} more."
            )
        else:
...
def cleanup_old_notifications():
    ...
    try:
        ...
        deleted_count = VendorNotification.objects.filter(is_read=True, created_at__lt=cutoff_date).delete()[0]
...
def refresh_tor_descriptors():
    ...
    try:
        ...
        result = subprocess.run(
            ["sudo", "systemctl", "reload", "tor@default"], capture_output=True, text=True, timeout=30
        )
        ...
        if result.returncode == 0:
            ...
            log_check = subprocess.run(
                ["sudo", "tail", "-10", "/var/log/tor/tor.log"], capture_output=True, text=True, timeout=10
            )
