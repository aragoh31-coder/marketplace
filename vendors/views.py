...
def vendor_list(request):
    ...
    if search:
        vendors = vendors.filter(Q(user__username__icontains=search) | Q(description__icontains=search))
...
def subvendor_dashboard(request, subvendor):
    ...
    if subvendor.can_view_orders:
        ...
        context["pending_orders"] = vendor_orders.filter(status__in=["paid", "created"]).count()
...
def vacation_settings(request):
    ...
    form = VacationModeForm(
        initial={"vacation_message": vendor.vacation_message, "vacation_ends": vendor.vacation_ends}
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            sub_user = User.objects.create_user(
                username=full_username, password=password, email=f"{full_username}@subvendor.local"
            )
