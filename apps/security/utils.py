...
def generate_captcha_challenge():
    ...
    challenge_data = {"question": f"{num1} + {num2}", "answer": answer, "timestamp": time.time()}
...
def calculate_security_score(user, request_data=None):
    ...
    recent_logs = AuditLog.objects.filter(user=user, created_at__gte=timezone.now() - timezone.timedelta(days=7))
...
def detect_suspicious_patterns(user, action, request_data=None):
    ...
    recent_failures = AuditLog.objects.filter(
        user=user, action=action, details__success=False, created_at__gte=timezone.now() - timezone.timedelta(hours=1)
    ).count()
    ...
    recent_actions = AuditLog.objects.filter(
        user=user, action=action, created_at__gte=timezone.now() - timezone.timedelta(minutes=5)
    ).count()
    ...
    return {"risk_score": risk_score, "risk_factors": risk_factors, "requires_review": risk_score >= 40}
