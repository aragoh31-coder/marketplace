...
def send_withdrawal_notification(withdrawal_request):
    ...
    try:
        ...
        send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, [withdrawal_request.user.email], fail_silently=False)

        admin_subject = f"New Withdrawal Request #{withdrawal_request.id}"
        admin_message = render_to_string("wallets/emails/withdrawal_admin.txt", {"withdrawal": withdrawal_request})
...
def validate_withdrawal_security(user, amount, currency, address):
    ...
    previous_use = WithdrawalRequest.objects.filter(user=user, address=address, status="completed").exists()
